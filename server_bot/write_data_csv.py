...
def data_selection():
    #  в цикле проходит по списку и для каждой акции записывает файл csv
    for stock in stock_list:
        #  фильтр по которому дастаем даные из БД
        prise = Prise.query.filter(Prise.secid == stock).order_by(Prise.tradedate.desc()).all()
        #  открываем файл для записи в формате csv
        with open('..\server_bot\static\{}.csv'.format(stock), 'w', newline='') as f:
...
"""
date_stock.tradedate, date_stock.secid, date_stock.value, date_stock.opens, date_stock.low, \
                    date_stock.high, date_stock.close


'tradedate', 'secid', 'value', 'opens', 'low', 'high', 'close'
